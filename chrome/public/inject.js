...
(() => {
  ...
  `;
    ...
    const makeRequest = (word) => {
        ...
        return new Promise((res) => chrome.runtime.sendMessage({ URLaddress }, (response) => res(Object.assign(Object.assign({}, response), { word }))));
    };
    ...
    chrome.storage.sync.get(["turnedOn", "background", "text", "border"], ({ turnedOn, background, text, border }) => {
    ...
    chrome.storage.onChanged.addListener(({ turnedOn, background, text, border }) => {
        if (turnedOn) {
